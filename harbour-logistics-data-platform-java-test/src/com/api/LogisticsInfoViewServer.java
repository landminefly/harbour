...
public class LogisticsInfoViewServer {
     ...
     */
    public List<LogisticsInfoViewBean> query(Connection conn,String[] args,long pageNo){

        List<LogisticsInfoViewBean> query = null;
        try {
            query = new LogisticsInfoViewDAOImpl().query(conn, args, pageNo);
    ...
    public long queryCount(Connection conn,String[] args,long pageNo){
        long count = 0;
        try {
            count = new LogisticsInfoViewDAOImpl().queryCount(conn, args, pageNo);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        return count;
